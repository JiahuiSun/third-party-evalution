...
class STCell(nn.Module):
    ...
    def forward(self, x, edge_index, dtw_edge_index):
        ...
        if self.choice[1] == 1:
            ...
            # print("gamma_1 shape is ", sp_adp_1.shape)
            # np.save("journal_exp/sp_gamma_1.npy", torch.reshape(torch.sigmoid(sp_adp_1), (-1, self.node_num, self.graph_dim)).cpu())
            ...
            # np.save("journal_exp/sp_gamma_2.npy", torch.reshape(torch.sigmoid(sp_adp_2), (-1, self.node_num, self.graph_dim)).cpu())
            ...
            # np.save("journal_exp/sp_gamma_3.npy", torch.reshape(torch.sigmoid(sp_adp_3), (-1, self.node_num, self.graph_dim)).cpu())
            ...
            # np.save("journal_exp/sp_gamma_4.npy", torch.reshape(torch.sigmoid(sp_adp_4), (-1, self.node_num, self.graph_dim)).cpu())
